...
export function RiskDonutChart({ students }: RiskDonutChartProps) {
      ...
      return (
        <div className="rounded-lg border border-border bg-card px-3 py-2 text-xs shadow-xl">
          ...
          <p className="text-muted-foreground">
            {entry.value} student{entry.value !== 1 ? "s" : ""} ({total ? Math.round((entry.value / total) * 100) : 0}%)
          </p>
        </div>
      );
    }
    ...
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      <div className="relative h-56">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={data} cx="50%" cy="50%" innerRadius={60} outerRadius={88} paddingAngle={3} dataKey="value" strokeWidth={0}>
              {data.map((entry) => (
        ...
        {data.map((entry) => (
          <div key={entry.name} className="flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 rounded-full shrink-0" style={{ backgroundColor: riskColors[entry.name] }} />
